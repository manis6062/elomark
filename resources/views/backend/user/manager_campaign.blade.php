...
@section('sub_title' , 'Campaign Manager Permissions')
@section('content')
...
<div class="card-block">
  ...
  @if(Auth::user()->isClientAdministrator())
     ...
     @if($user->getAccounts->first())
      ...
      @if(Auth::user()->isAdministrator())
       <h5 class="text-center f-w-900 text-muted">User : <span class="">{{$user->firstname}} {{$user->surname}}</span> </h5>
      @endif
  </div>
</div>
  ...
  <table id="users" class="table table-striped table-bordered nowrap">
   ...
   <tbody>   
    ...
    <input type="hidden" name="user_id" value="{{$user->id}}">
     ...
     <td class="text-capitalize">{{$accounts->find($value->account_id)->name}}</td>

    <td>

   <input name="campaign_user_id"  type="checkbox" class="js-success dashboard-access" value="{{($value->campaign_user_id == $user->id)?0 : $user->id}}" {{($value->campaign_user_id == $user->id)?'checked' :''}}/>
   </td>
